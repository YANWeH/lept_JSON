/*************************************************************************
	> File Name: leptjson.c
	> Author: YWH
	> Mail: 925957192@qq.com
	> Created Time: Fri 15 Jul 2022 07:09:41 AM EDT
 ************************************************************************/
...
static void* lept_context_push(lept_context* c, size_t size)
{
    ...
    if(c->top + size >= c->size)
    {
        ...
        while(c->top + size >= c->size)
            c->size += c->size >> 1; /*将stack的容量扩充1.5倍*/
        c->stack = (char*)realloc(c->stack, c->size);
    }
    ret = c->stack + c->top; /*栈顶指针, 指向的是下一个位置*/
    c->top += size;
    return ret;
}
...
{
    assert(c->top >= size);
    return c->stack + (c->top -= size); /*返回缩减后的栈顶指针*/
}
...
{
    size_t head = c->top;      /*head记录当前栈顶位置*/
    unsigned u, u2;
    const char* p;
    EXPECT(c, '\"');
    p = c->json;
    for(;;)
    {
        char ch = *p++; /* 先ch=*p，然后p++，即指向下一位 */ 
        switch(ch)
        {
            case '\"':/*遇到第二个 双引号 则数据读取结束，将stack缓存中长度为len的string记录到v中*/
                *len = c->top - head;
...
static int lept_parse_object(lept_context* c, lept_value* v)
{
    ...
    for(;;)
    {
       ...
       size++;
       m.k = NULL;
       /* parse ws [comma | right-curly-bracket] ws */
       lept_parse_whitespace(c);
       if(*c->json == ',')
       {
           c->json++;
...
int lept_parse(lept_value* v, const char* json)
{
    ...
    assert(c.top == 0); /*判断是否将缓存的数据全部读完了*/
    free(c.stack);      /*释放动态分配的缓存*/
    return ret;
}
...
static void lept_stringify_value(lept_context* c, const lept_value* v)
{
    ...
    switch(v->type)
    {
        ...
        case LEPT_NUMBER:   
                            {
                            int length = sprintf((char*)lept_context_push(c, 32), "%.17g", v->u.n);
                            c->top -= 32 - length;
                            break;
                            }
